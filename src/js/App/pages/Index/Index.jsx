...
const getTimeTextList = function(
  hourRange = HOUR_RANGE_24,
  minuteOffset = DEFAULT_MINUTE_OFFSET
) {
...
class Index extends Component {
  ...
  componentDidMount() {
    this.timetable.addEventListener(
      'dblclick',
      this.createNewEventInstance.bind(this),
      false
    );
  }
  ...
  createNewEventInstance(e) {
    ...
    const x = lastModifiedEventInstanceIndex != -1
      ? eventInstanceList[lastModifiedEventInstanceIndex].x
      : 0;
    const y = lastModifiedEventInstanceIndex != -1
      ? eventInstanceList[lastModifiedEventInstanceIndex].y
      : 0;
    const w = lastModifiedEventInstanceIndex != -1
      ? eventInstanceList[lastModifiedEventInstanceIndex].w
      : 1;
    const h = lastModifiedEventInstanceIndex != -1
      ? eventInstanceList[lastModifiedEventInstanceIndex].h
      : 3;
    let isDraggable,
      isResizable,
      maxH,
  ...
  onDragStop(items, item) {
    // debugger;
    let { eventInstanceList, lastModifiedEventInstanceIndex } = this.state;
    let index = -1;
  ...
  render() {
    const resizeDragClass = cx(styles['resize-drag']);

    const resizeContainerClass = cx(styles['resize-container']);
    ...
    if (eventInstanceList.length) {
      eventInstanceListHtml = eventInstanceList.map((i, k) => {
        return <div key={i.i} className={`${styles['grid-item']}`}>{i.i}</div>;
      });
    }
      ...
      return (
        <div className={`${styles['dp-tbl-row']}`} key={k}>
          <div className={`${styles['dp-tbl']}`}>
            <div
              className={`${styles['dp-tbl-cel']} ${styles['timetable__time-item']} ${styles['va-middle']}`}
            >
              {i}
            </div>
          </div>
        ...
        <div id="fullpage">
          <div className="section intros" data-anchor="intros">
            ...
            <div className={`${styles['dp-tbl']} ${styles['timetable']}`}>
              <div
                className={`${styles['dp-tbl-cel']} ${styles['timetable__time']}`}
                style={{}}
              >
                <div
                  className={`${styles['dp-tbl']}`}
              ...
              <div
                className={`${styles['dp-tbl-cel']} ${styles['va-top']}`}
            ...
            <div id="outer-dropzone" className={`${styles['dropzone']}`}>
              #outer-dropzone
              <div id="inner-dropzone" className={`${styles['dropzone']}`}>
                #inner-dropzone
              </div>
            </div>
            ...
            <Select
              ...
              className="underline"
              placeholder={`新增`}
              onChange={() => this.handleDaysSelectChange('ww')}
              validationOption={{
                check: true,
                required: true,
                showMsg: true,
          ...
          <div className="section article" data-anchor="articles">
            <div className="title">
              <span className="text" /><span className="icon icon-more" />
            </div>
          </div>
